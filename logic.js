      ...
      function populateServiceFilter() {
        const services = [
          ...new Set(allData.map((b) => b.service).filter(Boolean)),
        ];
      ...
      function updateStats(d) {
        ...
        // Simulated revenue (you can replace with actual pricing logic)
        const revenue = todayCount * 75; // Avg $75 per booking
        q("#revenue").textContent = "$" + revenue;
      }

      function updateAnalytics(d) {
        // Average per day
        const uniqueDays = new Set(d.map((b) => b.timestamp?.split("T")[0]))
          .size;
        ...
        const hours = d
          .map((b) => {
            ...
            return new Date(b.timestamp).getHours();
          })
          .filter((h) => h !== null);
